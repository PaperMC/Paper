...
public final class MemoryKeys {
    ...
    private static MemoryKey.NonValued unvalued(@KeyPattern.Value final String name) {
        final MemoryKey memoryKey = Registry.MEMORY_KEY.getOrThrow(Key.key(Key.MINECRAFT_NAMESPACE, name));
        if (memoryKey instanceof MemoryKey.NonValued unvalued) {
            return unvalued;
        }
    ...
    private static <T> MemoryKey.Valued<T> valued(@KeyPattern.Value final String name) {
        final MemoryKey memoryKey = Registry.MEMORY_KEY.getOrThrow(Key.key(Key.MINECRAFT_NAMESPACE, name));
