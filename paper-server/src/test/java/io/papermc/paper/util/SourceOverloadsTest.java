...
class SourceOverloadsTest {
    ...
    private static final MethodOverload[] OVERLOADS = new MethodOverload[] {
        ...
        new MethodOverload(
            ...
            "addEffect",
            4,
            EntityPotionEffectEvent.Cause.class.describeConstable().orElseThrow(),
            ConstantDescs.CD_boolean
        ),
        new MethodOverload(
            LivingEntity.class,
        ...
        // TODO check Entity#isCollidable
    };
    ...
    public void checkOverloads(final ClassInfo owner, final MethodInfoList methodList, final MethodOverload expectedOverload) {
        assertEquals(1, methodList.size(), "%s.%s has multiple overloads (%d)".formatted(owner.getName(), expectedOverload.name(), methodList.size()));
        final MethodInfo method = methodList.getFirst();

        assertEquals(expectedOverload.paramCount(), method.getParameterInfo().length, "%s.%s doesn't have %d params".formatted(owner.getName(), expectedOverload.name(), expectedOverload.paramCount()));
        ...
        for (int i = 0, len = addedParams.length; i < len; i++) {
            String expectedType = addedParams[i].descriptorString();
            int paramIndex = expectedOverload.paramCount() - len + i;
            String actualType = currentParams[paramIndex].getDescriptor();
            assertEquals(expectedType, actualType, "%s.%s method has the wrong type for param at index %d".formatted(owner.getName(), expectedOverload.name(), paramIndex));
        }
    }
}
