...
public class CraftTrialSpawner extends CraftBlockEntityState<TrialSpawnerBlockEntity> implements TrialSpawner {
    ...
    protected void applyTo(TrialSpawnerBlockEntity blockEntity) {
        ...
        if (this.normalConfig != null || this.ominousConfig != null) {
            ...
            blockEntity.getTrialSpawner().config = blockEntity.getTrialSpawner().config.withConfigs(
                Optionull.mapOrDefault(this.normalConfig, config -> Holder.direct(config.toMinecraft()), blockEntity.getTrialSpawner().config.normal()),
                Optionull.mapOrDefault(this.ominousConfig, config -> Holder.direct(config.toMinecraft()), blockEntity.getTrialSpawner().config.ominous())
            );
        }
    }
    ...
    public void setCooldownLength(int ticks) {
        this.getSnapshot().getTrialSpawner().config = this.getSnapshot().getTrialSpawner().config.withTargetCooldownLength(ticks);
    }
    ...
    public void setRequiredPlayerRange(int requiredPlayerRange) {
        this.getSnapshot().getTrialSpawner().config = this.getSnapshot().getTrialSpawner().config.withRequiredPlayerRange(requiredPlayerRange);
    }
    ...
    @Override
    public void configure(TrialSpawnerConfig activeConfig) {
        this.getTrialData().reset(); // clear to repick from the spawn definition
        ...
        spawner.config = spawner.config.withConfigs(
            !spawner.isOminous() ? PaperTrialSpawnerConfig.bukkitToMinecraftHolder(activeConfig) : spawner.config.normal(),
            spawner.isOminous() ? PaperTrialSpawnerConfig.bukkitToMinecraftHolder(activeConfig) : spawner.config.ominous()
        );
    }
