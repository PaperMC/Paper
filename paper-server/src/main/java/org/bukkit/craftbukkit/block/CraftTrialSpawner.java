    ...
    protected void applyTo(TrialSpawnerBlockEntity blockEntity) {
        ...
        if (this.normalConfig != null || this.ominousConfig != null) {
            ...
            blockEntity.trialSpawner.config = blockEntity.trialSpawner.config.withConfigs(
                Optionull.mapOrDefault(this.normalConfig, config -> Holder.direct(config.toMinecraft()), blockEntity.trialSpawner.config.normal()),
                Optionull.mapOrDefault(this.ominousConfig, config -> Holder.direct(config.toMinecraft()), blockEntity.trialSpawner.config.ominous())
            );
        }
    }
