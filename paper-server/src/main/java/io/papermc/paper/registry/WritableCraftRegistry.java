...
public class WritableCraftRegistry<M, T extends Keyed, B extends PaperRegistryBuilder<M, ? extends T>> extends CraftRegistry<T, M> {
    ...
    public class ApiWritableRegistry implements WritableRegistry<T, B> {
        ...
        @Override
        public <VB extends B> void registerWith(final TypedKey<? extends T> key, final Consumer<RegistryBuilderFactory<T, VB>> value) {
            WritableCraftRegistry.this.register((TypedKey<T>) key, factory -> value.accept((RegistryBuilderFactory<T, VB>) factory), this.conversions);
        }
    }
}
