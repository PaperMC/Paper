...
public abstract class PaperParticleType<A, M extends ParticleOptions> extends HolderableBase<ParticleType<M>> implements io.papermc.paper.particle.ParticleType {
    ...
    private static <P, V> Function<P, V> unsupportedOperation() {
        final class Holder {
            private static final Function<?, ?> UNSUPPORTED_OPERATION = _ -> {
    ...
    private static final PaperTypedDataAdapters<ParticleType<?>> ADAPTERS = PaperTypedDataAdapters.<ParticleType<?>, PaperTypedDataCollector<ParticleType<?>>>create(
        ...
        collector -> {
            collector.dispatch(_ -> Converters.unvalued()).add(
                ...
                ParticleTypes.PAUSE_MOB_GROWTH,
                ParticleTypes.RESET_MOB_GROWTH,
                ParticleTypes.SULFUR_BUBBLES,
            ...
            final ConverterGetter<ItemParticleOption, ItemStack> item = (type, value) -> {
                return new ItemParticleOption(type, CraftItemStack.asTemplate(value));
            };
            ...
            final ConverterGetter<DustParticleOptions, Particle.DustOptions> dust = (_, value) -> {
                return new DustParticleOptions(value.getColor().asRGB(), value.getSize());
            };
            final ConverterGetter<DustColorTransitionOptions, Particle.DustTransition> dustTransition = (_, value) -> {
                return new DustColorTransitionOptions(value.getColor().asRGB(), value.getToColor().asRGB(), value.getSize());
            };
            final ConverterGetter<VibrationParticleOption, Vibration> vibration = (_, value) -> {
            ...
            final ConverterGetter<SculkChargeParticleOptions, Float> sculkCharge = (_, value) -> new SculkChargeParticleOptions(value);
            final ConverterGetter<ShriekParticleOption, Integer> shriek = (_, value) -> new ShriekParticleOption(value);
            final ConverterGetter<TrailParticleOption, Particle.Trail> trail = (_, value) -> {
                return new TrailParticleOption(CraftLocation.toVec3(value.getTarget()), value.getColor().asRGB(), value.getDuration());
            };
            final ConverterGetter<GeyserBaseParticleOptions, Particle.GeyserBase> geyserBase = (type, value) -> {
                return new GeyserBaseParticleOptions(type, value.getWaterBlocks(), value.getBurstImpulse());
            };
            final ConverterGetter<GeyserParticleOptions, Particle.Geyser> geyser = (type, value) -> {
                return new GeyserParticleOptions(type, value.getWaterBlocks());
            };
            ...
            register(collector, ParticleTypes.GEYSER, geyser::get);
            register(collector, ParticleTypes.GEYSER_PLUME, geyser::get);
            register(collector, ParticleTypes.GEYSER_BASE, geyserBase::get);
            register(collector, ParticleTypes.GEYSER_POOF, geyserBase::get);
        }
    );
