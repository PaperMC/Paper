...
public class PaperBiomeMobSpawning implements BiomeMobSpawning {
    ...
    public PaperBiomeMobSpawning(Holder<Biome> biome) {
        this.biome = biome;
    }
    ...
    public Map<SpawnCategory, List<MobData>> spawners() {
        return Maps.asMap(allSpawnCategories, category -> {
            WeightedList<MobSpawnSettings.SpawnerData> nms = this.biome.value().getMobSettings().getMobs(CraftSpawnCategory.toNMS(category));
    ...
    public Map<EntityType, SpawnCost> entityCost() {
        ...
        this.biome.value().getMobSettings().mobSpawnCosts.forEach((entityType, mobSpawnCost) -> {
