...
public sealed interface ApiVersion extends io.papermc.asm.versioned.ApiVersion<ApiVersion> permits ApiVersion.Legacy, ApiVersion.Modern, ApiVersion.None {
    ...
    ApiVersion ABSTRACT_CUBE_MOB = getOrCreateVersion("26.2");
    ...
    record Modern(int year, int drop, int hotfix) implements ApiVersion {

        private static final Comparator<Modern> COMPARATOR = Comparator
            .comparingInt(Modern::year)
            .thenComparingInt(Modern::drop)
            .thenComparingInt(Modern::hotfix);
        ...
        @Override
        public String getVersionString() {
            return this.year() + "." + this.drop() + "." + this.hotfix();
        }
    ...
    private static ApiVersion parse(final String versionString) {
        ...
        if (first == 1) {
            ...
            if (first < 26) {
                throw new IllegalArgumentException("Modern version string must be 26.x.x or higher");
            }
